...
EquivalenceModel* EQmodelfor(std::string name, std::string command) {
  ...
  command_t << command;
  CLASSLogger* mylog = new CLASSLogger("myEQ.log",0,0);
  if (name == "FBR_BakerRoss_MOX") {
    ...
    return new EQM_FBR_BakerRoss_MOX(mylog, w_235u, w_238pu, w_240pu, w_241pu, w_242pu,
                                     w_241am, w_eqfissil);
    ...
    return new EQM_FBR_MLP_Keff(mylog, TMVAWeightPath, keff_target, InformationFile);
    ...
    return new EQM_MLP_Kinf(mylog, WeightPath, NumOfBatch, InformationFile,
                            CriticalityThreshold);

  } else if (name == "PWR_MLP_MOX_Am") {
    return new EQM_PWR_MLP_MOX_AM(mylog, command.c_str());
  } else if (name == "PWR_MLP_MOX") {
    return new EQM_PWR_MLP_MOX(mylog, command.c_str());
  } else if (name == "PWR_POL_UO2") {
    return new EQM_PWR_POL_UO2(mylog, command.c_str());
  } else {
...
XSModel* XSmodelfor(std::string name, std::string command) {
  std::stringstream command_t;
  command_t << command;
  CLASSLogger* mylog = new CLASSLogger("myXS.log",0,0);
  ...
  if (name == "XSM_MLP") {
    ...
    return new XSM_MLP(mylog, TMVA_Weight_Dir.c_str(), Info.c_str(), istime);
...
IrradiationModel* IMmodelfor(std::string name, std::string command) {
  CLASSLogger* mylog = new CLASSLogger("myIR.log",0,0);
  if (name == "RK4") {
    return new IM_RK4(mylog);
  } else if (name == "MATRIX") {
    return new IM_Matrix(mylog);
  } else {
    stringstream msg;
...
CLASSAdaptator::CLASSAdaptator(std::string EQModel, std::string EQcommand) {
  cyDBGL myPhysicsModel = new PhysicsModels();

  myPhysicsModel->SetEquivlalenceModel(EQmodelfor(EQModel, EQcommand));

  IsotopicVector IV_fissil =
      myPhysicsModel->GetEquivalenceModel()->GetStreamList("Fissile");
  ...
  fertil_list = Composition::CreateFromAtom(CLASS2CYCLUS(
      myPhysicsModel->GetEquivalenceModel()->GetStreamList("Fertile")));
  cyDBGL
}
...
float CLASSAdaptator::GetEnrichment(cyclus::Composition::Ptr c_fissil,
  ...
  cyDBGL IsotopicVector IV_fertil = CYCLUS2CLASS(c_fertil);
  cyDBGL
      ...
      // IV_fertil.Print();
      map<string, IsotopicVector>
          mymap;
  mymap["Fissile"] = IV_fissil;
  mymap["Fertile"] = IV_fertil;
  cyDBGL

      val = myPhysicsModel->GetEquivalenceModel()->GetMolarFraction(
          mymap, BurnUp)["Fissile"];
  cyDBGL return val;  //
}
...
float CLASSAdaptator::GetBU(cyclus::Composition::Ptr fuel, double eps) const {
  ...
  cyclus::Composition::Ptr fuel_fissil =
      ExtractAccordinglist(fuel, fissil_list);
  cyclus::Composition::Ptr fuel_fertil =
      ExtractAccordinglist(fuel, fertil_list);
  cyDBGL if (std::abs(AtomIn(fuel_fertil) + AtomIn(fuel_fissil) -
                      AtomIn(fuel)) > 1e-10) {
    std::stringstream msg;
    cout << "You fuel has nuclei that this model could not manage.."
         << std::endl;
    cout << "Missing "
         << std::abs(AtomIn(fuel_fertil) + AtomIn(fuel_fissil) - AtomIn(fuel));
    ...
    cout << "Fertile: " << AtomIn(fuel_fertil)
         << " Fissile: " << AtomIn(fuel_fissil) << " Total: " << AtomIn(fuel)
         << endl;
    exit(1);
  }

  cyDBGL float rho_target = AtomIn(fuel_fissil) / AtomIn(fuel);
  ...
  while (std::abs(rho_target - rho_estimated) /
             (rho_target / 2. + rho_estimated / 2.) >
         eps)
    ;
...
cyclus::Composition::Ptr CLASSAdaptator::GetCompAfterIrradiation(
  ...
  IsotopicVector AfterIrradiationIV =
      myEvolution.GetIsotopicVectorAt(finaltime);
...
cyclus::Composition::Ptr ExtractAccordinglist(cyclus::Composition::Ptr source,
  ...
  cyDBGL CompMap ListComp = list->atom();
  cyDBGL
